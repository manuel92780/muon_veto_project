...
parser.add_argument('-v','--version', default='23', type=str, dest = 'VER')
parser.add_argument('--emin', default=1e3, dest = 'EMIN')
parser.add_argument('--emax', default=1e5, dest = 'EMAX')
args = parser.parse_args()

plot_dir="/home/msilva/public_html/corsika_vs_muongun/Sibyll"+args.VER+"/depth_zenith/"
...
for zbin in range(len(zbins)-1):
    ...
    cor_r12_zbin=cor_r12[cor_zbins]; cor_r13_zbin=cor_r13[cor_zbins]; cor_r23_zbin=cor_r23[cor_zbins];
    cor_r14_zbin=cor_r14[cor_zbins]; cor_r24_zbin=cor_r24[cor_zbins]; cor_r34_zbin=cor_r34[cor_zbins];
    ...
    mug_r12_zbin=mug_r12[mug_zbins]; mug_r13_zbin=mug_r13[mug_zbins]; mug_r23_zbin=mug_r23[mug_zbins];
    mug_r14_zbin=mug_r14[mug_zbins]; mug_r24_zbin=mug_r24[mug_zbins]; mug_r34_zbin=mug_r34[mug_zbins];
    ...
    for bin in range(loop_bin):
        ...
        ax1.errorbar(bincenters, cor_2Dhist.T[bin],  yerr = np.sqrt(cor_errors_1.T[bin]), 
                     label=str(dbins[bin]) + ', ' + str(dbins[bin+1]) + " m",
                     fmt='o'+colors[bin], ecolor=colors[bin], drawstyle = 'steps-mid-')
        ax1.errorbar(bincenters, mug_2Dhist.T[bin],  yerr = np.sqrt(mug_errors_1.T[bin]),
    ...
    ax1.set_ylabel('Events per second [Hz]')
    ax2.set_ylabel('Corsika/ MuonGun')
    plt.savefig(plot_dir+"corsika_div_muongun_leadingenergy_binned_coszenith_"+lowz+"_"+highz+"_binned_depth.pdf")
    ...
    #lateral separation r1-2
    cor_2Dhist, xbins, ybins = np.histogram2d(cor_r12_zbin, cor_depth_zbin, bins=[rbins, dbins], weights=cor_weights_zbin)
    mug_2Dhist, xbins, ybins = np.histogram2d(mug_r12_zbin, mug_depth_zbin, bins=[rbins, dbins], weights=mug_weights_zbin)
    cor_errors_1, _, _       = np.histogram2d(cor_r12_zbin, cor_depth_zbin, bins=[rbins, dbins], weights=cor_errors_zbin)
    mug_errors_1, _, _       = np.histogram2d(mug_r12_zbin, mug_depth_zbin, bins=[rbins, dbins], weights=mug_errors_zbin)
    loop_bin = len(cor_2Dhist[0])
    for bin in range(loop_bin):
        print "now loop over depth bin: " + str(dbins[bin]) + ', ' + str(dbins[bin+1]) + " m"
        bincenters = 0.5*(xbins[1:]+xbins[:-1])
    ...
    ax1.set_ylabel('Events per second [Hz]')
    ax2.set_ylabel('Corsika/ MuonGun')
    plt.savefig(plot_dir+"corsika_div_muongun_rpos12_binned_coszenith_"+lowz+"_"+highz+"_binned_depth.pdf")
    print "made file: " + plot_dir+"corsika_div_muongun_rpos12_binned_coszenith_"+lowz+"_"+highz+"_binned_depth.pdf"
    plt.close("All")

    #lateral separation r1-2
    cor_2Dhist, xbins, ybins = np.histogram2d(cor_r13_zbin, cor_depth_zbin, bins=[rbins, dbins], weights=cor_weights_zbin)
    mug_2Dhist, xbins, ybins = np.histogram2d(mug_r13_zbin, mug_depth_zbin, bins=[rbins, dbins], weights=mug_weights_zbin)
    cor_errors_1, _, _       = np.histogram2d(cor_r13_zbin, cor_depth_zbin, bins=[rbins, dbins], weights=cor_errors_zbin)
    mug_errors_1, _, _       = np.histogram2d(mug_r13_zbin, mug_depth_zbin, bins=[rbins, dbins], weights=mug_errors_zbin)
    loop_bin = len(cor_2Dhist[0])
    for bin in range(loop_bin):
        print "now loop over depth bin: " + str(dbins[bin]) + ', ' + str(dbins[bin+1]) + " m"
        bincenters = 0.5*(xbins[1:]+xbins[:-1])
    ...
    ax1.set_ylabel('Events per second [Hz]')
    ax2.set_ylabel('Corsika/ MuonGun')
    plt.savefig(plot_dir+"corsika_div_muongun_rpos13_binned_coszenith_"+lowz+"_"+highz+"_binned_depth.pdf")
    print "made file: " + plot_dir+"corsika_div_muongun_rpos13_binned_coszenith_"+lowz+"_"+highz+"_binned_depth.pdf"
    plt.close("All")

    #lateral separation r2-3
    cor_2Dhist, xbins, ybins = np.histogram2d(cor_r23_zbin, cor_depth_zbin, bins=[rbins, dbins], weights=cor_weights_zbin)
    mug_2Dhist, xbins, ybins = np.histogram2d(mug_r23_zbin, mug_depth_zbin, bins=[rbins, dbins], weights=mug_weights_zbin)
    cor_errors_1, _, _       = np.histogram2d(cor_r23_zbin, cor_depth_zbin, bins=[rbins, dbins], weights=cor_errors_zbin)
    mug_errors_1, _, _       = np.histogram2d(mug_r23_zbin, mug_depth_zbin, bins=[rbins, dbins], weights=mug_errors_zbin)
    loop_bin = len(cor_2Dhist[0])
    for bin in range(loop_bin):
        print "now loop over depth bin: " + str(dbins[bin]) + ', ' + str(dbins[bin+1]) + " m"
        bincenters = 0.5*(xbins[1:]+xbins[:-1])
    ...
    ax1.set_ylabel('Events per second [Hz]')
    ax2.set_ylabel('Corsika/ MuonGun')
    plt.savefig(plot_dir+"corsika_div_muongun_rpos23_binned_coszenith_"+lowz+"_"+highz+"_binned_depth.pdf")
    print "made file: " + plot_dir+"corsika_div_muongun_rpos23_binned_coszenith_"+lowz+"_"+highz+"_binned_depth.pdf"
    plt.close("All")
