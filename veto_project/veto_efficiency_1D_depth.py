...
for frame in infile:
    ...
    if ("I3MCTree" in frame) and ("EntryMuon" in frame): #count all generated events
        event_count += 1; 
        number_muons = 0;
        energy_1 = 0; depth_1 = 0; weight_1 = 0; veto_1  = 0;
        for particle in frame["I3MCTree"]:
            ...
            if(energy > energy_1):
                 ...
                 depth_1 =  particle.pos.z                 #find depth
                 weight_1 = frame["MuonWeight"].value/normalization #normalized by total files
                 if("VHESelfVeto_"+args.PE in frame): veto_1  = (int(frame["VHESelfVeto_"+args.PE].value) ) * weight_1;
        if(number_muons > 1): continue #only care about n = 1 for now
        for i in range(len(depths)-1):
...
fit_x = []; fit_y = [];
for i in range(len(depths)-1):
     popt, pcov  = curve_fit(sigmoid, xvals[i], yvals[i])
